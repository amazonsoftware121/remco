...
class Throttler implements ThrottlerInterface
{
    ...
    public function check(string $key, int $capacity, int $seconds, int $cost = 1)
    {
        ...
        if (($tokens = $this->cache->get($tokenName)) === null)
        {
            ...
            return ['status' => true, 'value' => $capacity, 'new'=> true];
        }
        ...
        $remainingTime = $seconds - (int) $elapsed;

        if ($tokens >= 1)
        {
            $this->cache->save($tokenName, $tokens - $cost, $remainingTime);
            $this->cache->save($tokenName . 'Time', time(), $remainingTime);

            if((int) $tokens==1){
                return ['status' => false, 'time' => $remainingTime, 'value' => (int) $tokens];
            }

            return ['status' => true, 'value' => (int) $tokens];
