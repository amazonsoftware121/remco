...
export class CampaignLearnAndEarnComponent implements OnInit {
  ...
  getData(){
    this.web.getData('getevents?start='+this.start+'&end='+this.end +'&type='+'Learn').then((res)=>{
      if(res.status =='200'){
        this.learn=res.data
  ...
  //   this.web.getData('getActiveIdLearn?company_id='+ user_id + '&random_key='+user_random_key).then((res) => {
  //     if (res.status == '200') {
  ...
  async openPageView(data: any){
    ...
    let user_random_key=localStorage.getItem('user_random_key')
    await this.web.getData('getActiveIdLearn?company_id='+ user_id + '&random_key='+data.web_id).then((res) => {
      if (res.status == '200') {
        this.user_id = res.data[0];
        console.log('@@@@@@@@@@@@@@@@@@@@',this.user_id.campaign_id);
    ...
    // let restriction_id=localStorage.getItem('restriction_campaign_id');
