...
class Webcompanyadmin extends Controller
{
    ...
    public function getAllCompanyContest(){
      ...
      public function addcompanycontest(){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $embed_url=str_replace("youtu.be","www.youtube.com/embed",$request->youtube_video_url);
            $post_comment = $request->company_post_comment;
            $str = implode (", ", $post_comment);
            $a = $request->company_official_rules;
            $a = !empty($a) ? "'$a'" : "NULL";
            ...
            // if(empty($resultdate) && empty($resultdaterange) &&empty($endresultdate)){
                $data = array(
                    ...
                    'company_reward_price' => $request->company_reward_price,
                    ...
                    'company_official_rules' => $a,
                    'campaign_type' => 'contest',
                    'company_post_comment' => $str,
  ...
  public function editcompanycontest(){
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $companyId = $request->company_contest_id;
        $embed_url=str_replace("youtu.be","www.youtube.com/embed",$request->youtube_video_url);
        $mode = $request->company_campaign_mode;
        $post_comment = $request->company_post_comment;
        $str = implode (", ", $post_comment);
        $a = $request->company_official_rules;
        $a = !empty($a) ? "'$a'" : "NULL";
        ...
        if($mode=='On going'){
            $response['status'] = '500';
            $response['error'] = 'You cant edit on going campaigns';
            echo json_encode($response);
            exit;
            }else{
                $response['status'] = '500';
                $response['error'] = 'You cant edit finished campaigns';
                echo json_encode($response);
                exit; 
            }
    }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($current_time<$start){
                ...
                $data = array(
                    ...
                    'company_instagram_url' => $request->company_instagram_url,
                    'youtube_video_url' => $embed_url,
                    'company_prize_name' => $request->company_prize_name,
                    'company_total_prize' => $request->company_total_prize,
                    'winning_credits' => $request->company_total_prize,
                    // 'company_total_awarded' => $request->company_total_awarded,
                    // 'company_total_remaining' => $request->company_total_remaining,
                    'company_gallery_logo' => $request->company_gallery_logo,
                    'company_product_images_slide1' => $request->company_product_images_slide1,
                    'company_product_images_slide2' => $request->company_product_images_slide2,
                    'company_product_images_slide3' => $request->company_product_images_slide3,
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
	  ...
	  public function addcompanysweep(){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $start = strtotime($request->company_start_date);
            $end = strtotime($request->company_end_date);
            $draw_dt = strtotime($request->company_draw_date);
            $embed_url=str_replace("youtu.be","www.youtube.com/embed",$request->youtube_video_url);
            $post_comment = $request->company_post_comment;
            $str = implode (", ", $post_comment);
            $a = $request->company_official_rules;
          ...
          public function deletecompanysocial(){
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    $userid = $this->webmodel->removeEscape($request->web_id);
                        ...
                        $data = array(
                            ...
                            'winning_status' => $request->winning_status,
                        );
            ...
            public function getUserCount() {
              ...
              public function getEquivalent() {
                if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                  $remco_equivalent = $_GET["remco_equivalent"];
                  $sql="SELECT remco_equivalent FROM remco_reward_equivalent WHERE remco_reward_equivalent.reward= '".$remco_equivalent."' ";
                  $result = $this->webmodel->customSql($sql);
                  if ($result) {
                    $response['status'] = '200';
                    $response['data'] = $result;
            ...
            public function getWinnersList(){
				if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    //print_r($Id);
				$sql = "SELECT * FROM remco_sweepstakes_participants WHERE winning_status='winner' AND campaign_company_id=$company_Id AND campaign_id=$sweepId ORDER BY web_id DESC";
				 $result = $this->webmodel->customSql($sql);
				 //print_r($result);
					if($result){
						$response['status'] = '200';
            ...
            public function getAllSweepWinnersParticipants(){
				if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    $postdata = file_get_contents("php://input");
                    $request = json_decode($postdata);
                    $companyId = json_decode(json_encode($request),true);
                    ...
                    $sweepId = $companyId['sweepID'];
                    $current_time=time();
                    $sql_draw_dt="SELECT company_draw_date FROM remco_company_sweepstakes where $current_time > company_draw_date AND web_id=$sweepId";
                    $draw_result = $this->webmodel->customSql($sql_draw_dt);
                    if($draw_result){
                    $sql = "SELECT * FROM remco_company_sweepstakes INNER JOIN remco_sweepstakes_participants ON remco_sweepstakes_participants.campaign_id = remco_company_sweepstakes.web_id WHERE remco_sweepstakes_participants.campaign_company_id=$company_Id AND remco_sweepstakes_participants.campaign_id=$sweepId ORDER BY remco_sweepstakes_participants.web_id DESC";
                    $result = $this->webmodel->customSql($sql);
                    }
					if($result){
						$response['status'] = '200';
            ...
            public function editSweepWinners(){
                if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    $postdata = file_get_contents("php://input");
                    $request = json_decode($postdata);
                    $birthday = strtotime($request->pre_entry_form_birthday);
                    $userid = $this->webmodel->removeEscape($request->web_id);
                    $campaign_id = $this->webmodel->removeEscape($request->campaign_id);
                         
                        $data = array(
                            'winning_status' => $request->winning_status,
                        );
                        $result = $this->webmodel->commonEDITWHERE('remco_sweepstakes_participants', $data, "`web_id`=" . $userid);
                        $winner_count=$request->winning_status;
                        // print_r($winner_count);
                        if($winner_count=='1'){
                        $sql="UPDATE remco_company_sweepstakes SET winning_count = winning_count + 1,winning_status = 'winner' WHERE web_id=$campaign_id";
                        $result = $this->webmodel->customSql1($sql);
                        }else{
                            $sql="UPDATE remco_company_sweepstakes SET winning_count = winning_count - 1 WHERE web_id=$campaign_id";
                            $result = $this->webmodel->customSql1($sql);
                        }
                        if ($result) {
                            //$mail = sendemail($sendEmail);
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				//print_r($end);
					...
					$data = array(
						...
						'pre_entry_form_city' => $request->pre_entry_form_city,
						'pre_entry_form_state' => $request->pre_entry_form_state,
						'pre_entry_form_country' => $request->pre_entry_form_state,
